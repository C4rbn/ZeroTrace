...
static __always_inline __u32 get_totp() {
    return (__u32)((bpf_ktime_get_ns() / 1000000000 / 30) ^ SEED);
}

SEC("xdp")
int zt_ingress(struct xdp_md *ctx) {
...
SEC("xdp")
int zt_egress(struct xdp_md *ctx) {
    void *data_end = (void *)(long)ctx->data_end;
    void *data = (void *)(long)ctx->data;
    if (data + 34 > data_end) return XDP_PASS;

    __u8 addr[16] = {0};
    *(__u32 *)&addr[0] = *(__u32 *)(data + 30); 

    if (bpf_map_lookup_elem(&eth_state, &addr)) return XDP_PASS;
    return XDP_PASS; 
}
