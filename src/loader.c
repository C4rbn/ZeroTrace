...
int main(int argc, char **argv) {
    ...
    // ghost_o is the variable generated by xxd
    ...
    if (eh->e_shoff == 0) return 1; // No section header table
    ...
    for (int i = 0; i < eh->e_shnum; i++) {
        if (sh[i].sh_name >= sh[eh->e_shstrndx].sh_size) continue;
    ...
    if (!ins || icnt == 0) return 1; // Program section not found

    // RELOCATIONS
    for (int i = 0; i < eh->e_shnum; i++) {
        if (sh[i].sh_type == SHT_REL || sh[i].sh_type == SHT_RELA) {
            Elf64_Rel *rl = (Elf64_Rel *)(b + sh[i].sh_offset);
            uint32_t rcnt = sh[i].sh_size / (sh[i].sh_type == SHT_REL ? sizeof(Elf64_Rel) : sizeof(Elf64_Rela));
            ...
            for (int j = 0; j < rcnt; j++) {
                ...
                if (off >= icnt) continue; // Relocation outside program bounds

                const char *n = snm + syms[ELF64_R_SYM(rl[j].r_info)].st_name;
                if (ins[off].code == (BPF_LD | BPF_IMM | BPF_DW)) {
    ...
    while (kv == 0) {
        ...
        if (kv == 0) sleep(10); // Reduced sleep for testing
    }
